...
class TestCollection(unittest.TestCase):
    ...
    def test_create_index(self):
        ...
        self.assertEqual(db.system.indexes.find({"ns": "pymongo_test.test"})
                         .count(), 1)
        db.test.create_index([("hello", DESCENDING), ("world", ASCENDING)])
        self.assert_("hello_-1_world_1" in
                     [a["name"] for a in db.system.indexes
                      .find({"ns": "pymongo_test.test"})])
    ...
    def test_field_selection(self):
        ...
        l = list(next(db.test.find({}, ["a"])).keys())
        l.sort()
        self.assertEqual(l, ["_id", "a"])
        l = list(next(db.test.find({}, ["b"])).keys())
        l.sort()
        self.assertEqual(l, ["_id", "b"])
        l = list(next(db.test.find({}, ["c"])).keys())
        l.sort()
        ...
        l = list(next(db.test.find({}, ["b", "c.e"])).keys())
        l.sort()
    ...
    def test_find_w_fields(self):
        ...
        self.assertFalse("extra thing" in next(db.test.find({},
                         ["x", "mike"])))
        self.assertFalse("x" in next(db.test.find({}, ["mike"])))
        self.assert_("mike" in next(db.test.find({}, ["mike"])))
        self.assertFalse("extra thing" in next(db.test.find({}, ["mike"])))
    ...
    def test_as_class(self):
        ...
        self.assert_(isinstance(next(c.find()), dict))
        self.assertFalse(isinstance(next(c.find()), SON))
        self.assert_(isinstance(next(c.find(as_class=SON)), SON))
        ...
        self.assertEqual(1, next(c.find(as_class=SON))["x"])
