...
def _get_date(data, as_class, tz_aware):
    seconds = struct.unpack("<q", data[:8])[0] / 1000.0
    if tz_aware:
...
def _element_to_bson(key, value, check_keys):
    ...
    if isinstance(value, Binary):
        ...
        return b"\x05" + name + struct.pack("<i", len(value)) + bytes([subtype]) + value
...
def is_valid(bson):
    ...
    :class:`bytes`.  Returns ``True`` if `bson` is valid :class:`BSON`,
    ``False`` otherwise.
